...
CREATE TABLE "contatos"(
    ...
    "rg"                                VARCHAR(10),
    "inscricao_municipal"               VARCHAR(13),
    "orgao_emissor"                     VARCHAR(55),
    "email"                             VARCHAR(255),
    "id_vendedor"			INTEGER                       REFERENCES "vendedores"("id_bling"),
...
COMMENT ON COLUMN
    "produtos_midias"."dimensoes" IS '`0` Metros
    `1` Centímetros
    `2` Milímetros';
...
COMMENT ON COLUMN
    ...
    Valor default: `false`';
COMMENT ON COLUMN
    "produtos"."descricao_embalagem_discreta" IS 'Descrição discreta do produto para utilizar na declaração de conteúdo.';
COMMENT ON COLUMN
    "produtos"."situacao_produto" IS 'Ação de estoque ao transformar produto Simples em Variação
...
CREATE TABLE "tipos_pagamento"(
    "id"    SERIAL PRIMARY KEY  NOT NULL,
    "nome"  VARCHAR(127)         NOT NULL  CHECK ("nome" <> '')
);
COMMENT ON COLUMN
...
COMMENT ON COLUMN
    "contas_origem_situacoes"."id" IS 'Situações da venda:
    `0` Em aberto
    `1` Atendido
    `2` Cancelado
    `3` Em andamento
...
CREATE TABLE "vendas"(
    ...
    "id_loja"                       INTEGER             NOT NULL REFERENCES "canais_vendas"("id_bling"),
    ...
    "intermediador_cnpj"            VARCHAR(14)         NOT NULL,
    "intermediador_nome_usuario"    VARCHAR(63)         NOT NULL,
    "taxa_comissao"                 INTEGER             NOT NULL,
    "taxa_custo_frete"              INTEGER             NOT NULL,
    "taxa_valor_base"               INTEGER             NOT NULL,
    "alterado_em"                   TIMESTAMPTZ         DEFAULT current_timestamp
);
COMMENT ON COLUMN
...
COMMENT ON COLUMN
    "vendas"."taxa_comissao" IS 'Taxa de comissão perante ao total da venda.';
COMMENT ON COLUMN
    "vendas"."taxa_custo_frete" IS 'Valor de custo do frete.';
COMMENT ON COLUMN
    "vendas"."taxa_valor_base" IS 'Valor base da venda para demonstrativo de cálculo das taxas via interface (Se não informado considera o total da venda).'
...
CREATE TABLE "transporte_volumes"(
    ...
    "id_logistica_servico"  BIGINT              NOT NULL REFERENCES "logistica_servicos"("id_bling"),
...
CREATE TABLE "canais_vendas_filiais"(
    "id"                SERIAL PRIMARY KEY  NOT NULL,
    "id_canal_venda"    BIGINT              NOT NULL REFERENCES "canais_vendas"("id_bling"),
...
CREATE TABLE "logisticas"(
    ...
    "integracao_nativa" BOOLEAN             NOT NULL,
    "situacao"          BOOLEAN             NOT NULL,
    "integracao_id"     BIGINT 
);
COMMENT ON COLUMN
    "logisticas"."situacao" IS 'Situação do canal de venda
    `TRUE` Habilitado
    `FALSE` Desabilitado';

---*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*

CREATE TABLE "logistica_servicos"(
    "id_bling"              BIGINT PRIMARY KEY  NOT NULL,
    "descricao"             VARCHAR(255)        NOT NULL,
...
CREATE TABLE "logistica_servico_aliases"(
    "id"            INTEGER PRIMARY KEY NOT NULL,
    "servico_id"    BIGINT              NOT NULL REFERENCES "logistica_servicos"("id_bling"),
